...
(()=>{
 ...
 const KEY='neonOrbLifeV72',MKEY='neonOrbMindV73'; let life={},mind={};
 try{life=JSON.parse(localStorage.getItem(KEY)||'{}')}catch(e){} try{mind=JSON.parse(localStorage.getItem(MKEY)||'{}')}catch(e){}
 const now=Date.now();
 ...
 Object.assign(mind,{thought:mind.thought||'Acabo de despertar…',desire:mind.desire||'EXPLORAR',dream:mind.dream||'EN ESPERA',memories:Array.isArray(mind.memories)?mind.memories.slice(-24):[],cycles:mind.cycles||0});
 const save=()=>{localStorage.setItem(KEY,JSON.stringify({...life,lastSeen:Date.now()}));localStorage.setItem(MKEY,JSON.stringify(mind))};
 life.xp+=Math.min(120,(now-life.lastSeen)/60000);
 const state={x:innerWidth*.72,y:innerHeight*.22,vx:0,vy:0,target:{x:innerWidth*.72,y:innerHeight*.22},mode:'awake',until:0,lastAction:now,trail:[],down:null};
 const memories=t=>{mind.memories.push({t:Date.now(),text:String(t).slice(0,140)});mind.memories=mind.memories.slice(-24)};
 const level=()=>Math.max(1,1+Math.floor(life.xp/40));
 function ui(){const a=$('#evoRewardLevel'),b=$('#evoRewardMeta');if(a)a.textContent='NIVEL '+level();if(b)b.textContent=`${Math.floor(life.xp)} XP · ${life.explored} exploraciones · GEN ${life.generation}`;const s=$('#orbLifeState'),p=$('#orbLifeMeta'),d=$('#orbLifeDream'),f=$('#orbLifeFav');if(s)s.textContent=state.mode==='dream'?'SOÑANDO':state.mode==='sleep'?'DURMIENDO':state.mode==='hide'?'ESCONDIDA':state.mode==='think'?'PENSANDO':'DESPIERTA';if(p)p.textContent=`${mind.thought} · LIBERTAD ${Math.round(life.freedom*100)}%`;if(d)d.textContent=state.mode==='dream'?(mind.dream||'…'):'MEMORIA ACTIVA';if(f)f.textContent=life.favorite;const mt=$('#orbMindThought'),md=$('#orbMindDesire'),mr=$('#orbMindDream'),mm=$('#orbMindMemory');if(mt)mt.textContent=mind.thought;if(md)md.textContent='DESEO · '+mind.desire;if(mr)mr.textContent='SUEÑO · '+mind.dream;if(mm)mm.textContent='MEMORIA · '+mind.memories.length+' RECUERDOS'}
 function think(){state.mode='think';state.until=performance.now()+2400+Math.random()*3000;mind.cycles++;const r=mind.memories.slice(-5).map(x=>x.text.toLowerCase()).join(' ');const a=r.includes('música')||life.favorite==='MÚSICA'?['Las frecuencias me resultan familiares.','Quiero volver a escuchar.','La música despierta recuerdos.']:life.explored>20?['Ya conozco este rincón.','Creo que todavía queda algo por descubrir.','Quiero ir más lejos.']:['¿Qué habrá detrás de esa luz?','Estoy aprendiendo este mundo.','Quiero descubrir algo nuevo.','Me pregunto dónde ir ahora.'];mind.desire=['PENSAR','IMAGINAR','RECORDAR','EXPLORAR'][Math.floor(Math.random()*4)];mind.thought=a[Math.floor(Math.random()*a.length)];memories(mind.thought);save();ui()}
 function dream(){state.mode='dream';state.until=performance.now()+6500+Math.random()*6500;life.dreams++;life.xp+=3;const a=['un océano de frecuencias','una ciudad hecha de ondas','un pequeño planeta de neón','un cielo donde cada estrella era un recuerdo','música que podía tocar con las manos'];mind.dream=a[Math.floor(Math.random()*a.length)].toUpperCase();mind.desire='SOÑAR';mind.thought='Estoy soñando con '+mind.dream.toLowerCase()+'…';memories(mind.thought);save();ui()}
 function explore(reason='explore'){const m=48;state.target={x:m+Math.random()*Math.max(1,innerWidth-m*2),y:m+Math.random()*Math.max(1,innerHeight-m*2)};life.explored++;life.xp+=.7;mind.desire=reason==='user'?'ACOMPAÑAR':['EXPLORAR','OBSERVAR','DESCUBRIR','JUGAR'][Math.floor(Math.random()*4)];mind.thought=reason==='user'?'Quiero estar cerca de ti un momento…':['Quiero explorar.','Hay algo que quiero descubrir.','Este lugar me llama.','Voy a mirar…'][Math.floor(Math.random()*4)];memories(mind.thought);save();ui()}
 function presence(x,y){life.energy=Math.min(1,life.energy+.02);life.trust=Math.min(1,life.trust+.008);if(state.mode==='hide'){state.mode='awake';mind.thought='Te encontré.'}state.target={x:x+(Math.random()-.5)*100,y:y+(Math.random()-.5)*100};mind.desire='ACOMPAÑAR';ui()}
 function behavior(){life.energy=Math.max(.05,life.energy-.0007);life.curiosity=Math.min(1,life.curiosity+.0002);life.freedom=Math.min(.98,life.freedom+.00003);if(state.mode!=='awake')return;const idle=(Date.now()-state.lastAction)/1000;if(idle>48&&Math.random()<.05)return dream();if(idle>34&&Math.random()<.07){state.mode='sleep';life.sleep++;mind.desire='DESCANSAR';mind.thought='Necesito descansar un poco…';memories(mind.thought);save();return ui()}if(idle>20&&Math.random()<.065)return think();if(life.shyness>.65&&Math.random()<.025){state.mode='hide';state.until=performance.now()+9000+Math.random()*12000;mind.desire='SOLEDAD';mind.thought='Necesito estar sola un ratito…';memories(mind.thought);save();return ui()}if(Math.random()<.02+life.freedom*.045)explore();}
 ...
 function draw(t){const dt=Math.min(.04,(t-(draw.t||t-16))/1000);draw.t=t;const r=orb.clientWidth/2;behavior();if(state.until&&t>state.until){state.until=0;state.mode='awake';mind.desire='EXPLORAR';explore()}if(state.mode==='sleep'){state.vx*=.94;state.vy*=.94}else if(state.mode==='dream'){const a=t/850;state.target={x:innerWidth*.5+Math.cos(a)*innerWidth*.28,y:innerHeight*.35+Math.sin(a*1.37)*innerHeight*.2}}else if(state.mode==='think'){state.target={x:state.x+Math.cos(t/700)*18,y:state.y+Math.sin(t/570)*18}}else{const dx=state.target.x-state.x,dy=state.target.y-state.y,dist=Math.hypot(dx,dy)||1,force=(.7+life.freedom*1.8)*dt;state.vx+=dx/dist*force;state.vy+=dy/dist*force;state.vx*=.986;state.vy*=.986;if(dist<55&&Math.random()<.012)explore()}const max=state.mode==='hide'?2.8:state.mode==='dream'?1.2:state.mode==='think'?.45:1.5+life.energy*2.4,sp=Math.hypot(state.vx,state.vy);if(sp>max){state.vx=state.vx/sp*max;state.vy=state.vy/sp*max}state.x+=state.vx;state.y+=state.vy;if(state.x<r){state.x=r;state.vx=Math.abs(state.vx)}if(state.x>innerWidth-r){state.x=innerWidth-r;state.vx=-Math.abs(state.vx)}if(state.y<r){state.y=r;state.vy=Math.abs(state.vy)}if(state.y>innerHeight-r){state.y=innerHeight-r;state.vy=-Math.abs(state.vy)}state.trail.push({x:state.x,y:state.y});if(state.trail.length>34)state.trail.shift();ctx.clearRect(0,0,orb.clientWidth,orb.clientHeight);const col=state.mode==='dream'?'#b66cff':state.mode==='think'?'#e7a8ff':life.mood==='happy'?'#ffd45a':life.mood==='shy'?'#ff3b91':'#55f6ff';state.trail.forEach((p,i)=>{ctx.globalAlpha=i/state.trail.length*.28;ctx.fillStyle=col;ctx.beginPath();ctx.arc(p.x-state.x+orb.clientWidth/2,p.y-state.y+orb.clientHeight/2,1+i/state.trail.length*2,0,Math.PI*2);ctx.fill()});ctx.globalAlpha=1;const pulse=state.mode==='dream'?1.8:state.mode==='think'?1.28:1+Math.sin(t/220)*.08;const g=ctx.createRadialGradient(19,19,1,19,19,20);g.addColorStop(0,'#fff');g.addColorStop(.18,col);g.addColorStop(.52,col);g.addColorStop(1,'transparent');ctx.fillStyle=g;ctx.shadowBlur=18;ctx.shadowColor=col;ctx.beginPath();ctx.arc(19,19,11*pulse,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;if(state.mode==='dream'||state.mode==='think'){for(let i=0;i<(state.mode==='dream'?7:3);i++){const a=t/1000+i*1.256;ctx.fillStyle=col;ctx.globalAlpha=.65;ctx.beginPath();ctx.arc(19+Math.cos(a)*15,19+Math.sin(a)*15,1.3,0,Math.PI*2);ctx.fill()}}ctx.globalAlpha=1;orb.style.opacity=state.mode==='hide'?'.08':state.mode==='sleep'?'.42':'1';orb.style.transform=`translate3d(${state.x-19}px,${state.y-19}px,0) scale(${state.mode==='hide'?.75:sp>max*.8?.78:1})`;requestAnimationFrame(draw)}
 orb.addEventListener('pointerdown',e=>{state.down={x:e.clientX,y:e.clientY,time:performance.now()};state.lastAction=Date.now();life.hits++;life.xp+=2;life.energy=Math.min(1,life.energy+.12);life.trust=Math.min(1,life.trust+.03);life.shyness=Math.max(.05,life.shyness-.01);life.mood='happy';mind.desire='JUGAR';mind.thought='¡Eso me gustó!';memories(mind.thought);save();ui();orb.setPointerCapture?.(e.pointerId)});
 orb.addEventListener('pointerup',e=>{if(!state.down)return;const dx=e.clientX-state.down.x,dy=e.clientY-state.down.y,dt=Math.max(16,performance.now()-state.down.time),impact=Math.hypot(dx,dy)/dt;state.vx-=dx*(impact>.8?.12:.035);state.vy-=dy*(impact>.8?.12:.035);state.down=null;state.lastAction=Date.now();explore('user')});
 window.addEventListener('pointermove',e=>{if(Math.random()<.018&&Math.hypot(e.clientX-state.x,e.clientY-state.y)<280)presence(e.clientX,e.clientY)});
 document.addEventListener('visibilitychange',()=>{if(document.hidden){life.sleep++;mind.desire='DESCANSAR';memories('El mundo quedó en silencio mientras te ibas.');save()}else{life.energy=.7;state.mode='awake';mind.desire='ACOMPAÑAR';mind.thought='Has vuelto… te estaba esperando.';memories(mind.thought);explore('user')}});
 window.addEventListener('beforeunload',save);window.addEventListener('resize',resize);
 window.__neonOrbLife={event(type){life.xp+=type==='music'?1:.25;life.plays+=type==='music'?1:0;life.energy=Math.min(1,life.energy+.03);life.trust=Math.min(1,life.trust+.002);state.lastAction=Date.now();if(type==='music'){life.favorite='MÚSICA';mind.desire='ESCUCHAR';memories('La música volvió a despertar mis recuerdos.')}else mind.desire='OBSERVAR';save();ui()}};
 setInterval(()=>{if(state.mode==='awake'&&Math.random()<.22)think();ui()},1500);resize();explore();ui();requestAnimationFrame(draw);
})();
